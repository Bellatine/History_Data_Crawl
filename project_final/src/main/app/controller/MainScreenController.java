...
import main.source.handler.CrawlDataHandle;
import main.source.handler.ReadDataHandle;
...
public class MainScreenController extends WindowController implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		for(King king : LoadCache.listKing){
			branchPerson.getChildren().add(new TreeItem<Subject>(king));
		}
		for(Dynasty dynasty : LoadCache.listDynasty){
			branchDynasty.getChildren().add(new TreeItem<Subject>(dynasty));
		}
		/*branchDynasty.getChildren().add(new TreeItem<Subject>(new ));
